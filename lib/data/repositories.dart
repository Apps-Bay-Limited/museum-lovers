...
import 'package:museumLovers/cache.dart';
...
class HttpRijksRepository implements RijksRepository {
  ...
  Future<List<Art>> fetchAll({required int page}) async {
    try {
      final Response response =
          await client.get(Uri.parse('$baseUrl?key=$apiKey&ps=$itemsPerPage&p=$page'));
      final dynamic data = json.decode(response.body);
      return <Art>[
  ...
  @override
  Future<List<Art>> fetchSearchAll({required String term, required int page}) async {
    try {
      final Response response =
          await client.get(Uri.parse('$baseUrl?key=$apiKey&q=$term&ps=$itemsPerPage&p=$page'));
      final dynamic data = json.decode(response.body);
      return <Art>[
        for (final Map<String, dynamic> item in data['artObjects']) Art.fromJson(item),
      ];
  ...
  Future<List<Art>> fetchAll({required int page}) async {
    ...
    return List<Art>.generate(20, (int index) {
      ...
      final String imageUrl =
          'https://via.placeholder.com/$imageSize/$backgroundColor/ffffff?text= ';
      return Art(
        objectNumber: '$index',
        id: '$index',
        hasImage: true,
